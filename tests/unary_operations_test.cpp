...
TEST_CASE("-x {3}", "UnaryMinus") {
  ...
  CHECK(result->GetInfix(0, {{'x', -3}}) == "-(-3)");
  CHECK(result->GetRPN({{'x', -3}}) == "3 ~ ~");
  REQUIRE_THAT(result->GetNumericResult({{'x', 5}}), Catch::Matchers::WithinRel(-5, 1e-5));
...
TEST_CASE("-(-x))", "UnaryMinus") {
  ...
  auto result = std::make_shared<math::UnaryMinus>(std::make_shared<math::UnaryMinus>(operand));
  auto derivative = result->GetDerivative();
...
TEST_CASE("-(-3))", "UnaryMinus") {
  ...
  CHECK(result->GetInfix(0, {}) == "-(-3)");
  CHECK(result->GetRPN({}) == "3 ~ ~");
